...
class StrikeDetector:
    ...
    def analyze(self, landmarks):
        ...
        return {
            ...
            "output_rate": round(len([s for s in self.strike_log if s["time_since"] < 60]) / 60.0 * self.fps, 1),
        }
    ...
    def _detect_strikes(self, pts):
        ...
        lead_shoulder = pts.get("l_shoulder") if pts.get("l_wrist", [0, 0, 0])[0] < pts.get("r_wrist", [0, 0, 0])[0] else pts.get("r_shoulder")
        rear_shoulder = pts.get("r_shoulder") if lead_shoulder == pts.get("l_shoulder") else pts.get("l_shoulder")
        lead_wrist = pts.get("l_wrist") if lead_shoulder == pts.get("l_shoulder") else pts.get("r_wrist")
        rear_wrist = pts.get("r_wrist") if lead_shoulder == pts.get("l_shoulder") else pts.get("l_wrist")
        ...
        hands = [("left", l_wrist, pts.get("l_elbow"), pts.get("l_shoulder")),
        ...
        for side, wrist, elbow, shoulder in hands:
            ...
            if self._is_forward(delta, shoulder, wrist) and speed > self.velocity_threshold * 1.2:
                if side == "left" if lead_wrist is not None and id(wrist) == id(lead_wrist) else side == "right":
                    strike_type = "jab"
                else:
                    strike_type = "cross"
        ...
        for side, ankle, knee, hip in [
            ...
            if speed < self.velocity_threshold * 0.8:
                continue
            knee_angle = self._angle_at(knee, hip, ankle)
            strike_type = None
            ...
            elif self._is_upward(delta) and knee_angle < 90:
                strike_type = "knee"
